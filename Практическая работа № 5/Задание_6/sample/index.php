...
<body>
    ...
    <?php
        ...
        foreach ($albums as $key => $item) {
            $key = $key < 9 ? "000" . ++$key : "00" . ++$key;
            
            $item['label'] = strip_tags($item['label']);
            $item['format'] = strip_tags($item['format']);
            $item['status'] = strip_tags($item['status']);
            ...
            $out .= sprintf("
                ...
                Статус: %s<p>
                <hr />
            ", $key, $item['id'], $item['album_name'], $item['date'], 
               $item['label'], $item['format'], $item['status']);
        };
